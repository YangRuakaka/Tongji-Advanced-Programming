...
public:
	...
	double area()
	{
		if (p1.x* p2.y* 1 + p2.x * p3.y * 1 + p3.x * p1.y * 1 - p1.x * p3.y * 1 - p2.x * p1.y * 1 - p3.x * p2.y * 1)
			return fabs((1.00 / 2) * (p1.x * p2.y * 1 + p2.x * p3.y * 1 + p3.x * p1.y * 1 - p1.x * p3.y * 1 - p2.x * p1.y * 1 - p3.x * p2.y * 1));
		else
			return -1;
	}
	/* 构造函数 */
